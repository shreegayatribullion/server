...
router.post('/contact', (req, res) => {
  ...
  const submit = async () => {
    ...
    if (body.action === "INSERT") {
      pool.query(
        `INSERT INTO shree_gayatri_bullion_contact ( name, cnumber, frim, msg ) 
      ...
      pool.query(
        `DELETE FROM shree_gayatri_bullion_contact WHERE id=${body.id};`,
        async function (err, data) {
          if (data && data.affectedRows) {
            // res.send(req.files)
            return res.status(200).json({
...
router.get('/contact', (req, res) => {
  ...
  const submit = async () => {
    ...
    sSQL = `SELECT *  FROM shree_gayatri_bullion_contact`;
    pool.query(
      sSQL,
      async function (err, data) {
        if (err) {
